...
SAR_TABLE_FILENAME = 'data/sar_table_v2.csv' # search-and-replace table
...
def process():
    ...
    with open(SAR_TABLE_FILENAME) as csvfile:
        ...
        for target_col, _, orig, transcribed in sar_reader:
            search_and_replace[(int(target_col), orig)] = transcribed
    ...
    for row in reader:
        ...
        for col, val in enumerate(row):
            ...
            # search-and-replace in surnames, firstnames, fathernames, counties and places #11
            if col in {1, 2, 3, 6, 8}:
                key_to_check = (col, val)
                if key_to_check in search_and_replace.keys():
                    val = search_and_replace[key_to_check]
